...
def process_func(path: str, aug_rate=1, missing_ratio=0.1, train=True, dataset_name = 'acic', current_id='0'):
    ...
    if dataset_name == 'acic2016':
        load_mask_path = "./data/acic2016/acic2016_mask/" + current_id + ".csv"
        print(load_mask_path)

    if dataset_name == 'acic2018':
        load_mask_path = "./data/acic2018/acic2018_mask/" + current_id + ".csv"
        print(load_mask_path)
    if dataset_name == 'synthetic':
        load_mask_path = "./data/synthetic_data/synthetic_mask/" + current_id + ".csv"
        print(load_mask_path)
...
class tabular_dataset(Dataset):
    def __init__(
        self, use_index_list=None, aug_rate=1, missing_ratio=0.1, seed=0, train=True, dataset_name = 'acic', current_id='0'
    ):  
        if dataset_name == 'acic2016':
            self.eval_length = 87
        if dataset_name == 'acic2018':
        ...
        if dataset_name == 'acic2016':
            ...
            processed_data_path = (
                f"./data/acic2016/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data/acic2016/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))

        if dataset_name == 'acic2018':
            dataset_path = "./data/acic2018/acic2018_norm_data/" + current_id + ".csv"
            print('dataset_path', dataset_path)
            processed_data_path = (
                f"./data/acic2018/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data/acic2018/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))
        if dataset_name == 'synthetic':
            dataset_path = "./data/synthetic_data/synthetic_norm_data/" + current_id + ".csv"
            print('dataset_path', dataset_path)
            processed_data_path = (
                f"./data/synthetic_data/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data/synthetic_data/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, dataset_name = 'acic2018', current_id='0'):
    ...
    if dataset_name == 'acic2016':
        ...
        processed_data_path_norm = (
            f"./data/acic2016/missing_ratio-{missing_ratio}_seed-{seed}_current_id-{current_id}_max-min_norm.pk"
        )
        print(
    ...
    if dataset_name == 'acic2018':
        ...
        processed_data_path_norm = (
            f"./data/acic2018/missing_ratio-{missing_ratio}_seed-{seed}_current_id-{current_id}_max-min_norm.pk"
        )
        print(
            "------------- Perform data normalization and store the mean value of each column.--------------"
        )
    if dataset_name == 'synthetic':
        ...
        train_index = remain_index[: tsi]
        valid_index = indlist[: int(tsi*0.1)]

        processed_data_path_norm = (
            f"./data/synthetic_data/missing_ratio-{missing_ratio}_seed-{seed}_current_id-{current_id}_max-min_norm.pk"
        )
        print(
            "------------- Perform data normalization and store the mean value of each column.--------------"
        )
