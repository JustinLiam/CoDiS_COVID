    ...
    for epoch_no in range(config["epochs"]):
        ...
        if valid_loader is not None and (epoch_no + 1) % valid_epoch_interval == 0:
            ...
            with torch.no_grad():
                ...
                wandb.log({"y0 VAL RMSE": y0, "y1 VAL RMSE": y1, "PEHE VAL": pehe})
...
def evaluate(model, test_loader, nsample=100, scaler=1, mean_scaler=0, foldername=""):
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            ...
            wandb.log({"y0 TEST RMSE": y0, "y1 TEST RMSE": y1, "PEHE TEST": pehe})
...
#---------------uncertainty estimation-------------------------
            ...
            try:
                ...
                print(f'Calculating Test Prediction Intervals for {pred_samples_y0.shape[0]} points...')
                prob_0_95, median_width_0_95 = compute_interval(confidence_level=0.95, po_samples=pred_samples_y0,
                                                                y_true=truth_y0)
                prob_1_95, median_width_1_95 = compute_interval(confidence_level=0.95, po_samples=pred_samples_y1,
                                                                y_true=truth_y1)
                prob_0_99, median_width_0_99 = compute_interval(confidence_level=0.99, po_samples=pred_samples_y0,
                                                                y_true=truth_y0)
                prob_1_99, median_width_1_99 = compute_interval(confidence_level=0.99, po_samples=pred_samples_y1,
                                                                y_true=truth_y1)
                print(f"Test Coverage Prob Y0 (95%): {prob_0_95:.4f}, Median Width Y0: {median_width_0_95:.4f}")
                print(f"Test Coverage Prob Y1 (95%): {prob_1_95:.4f}, Median Width Y1: {median_width_1_95:.4f}")
                print(f"Test Coverage Prob Y0 (99%): {prob_0_99:.4f}, Median Width Y0: {median_width_0_99:.4f}")
                print(f"Test Coverage Prob Y1 (99%): {prob_1_99:.4f}, Median Width Y1: {median_width_1_99:.4f}")
                wandb_log["prob0_TEST(0.95)"] = prob_0_95
                wandb_log["median_width0_TEST(0.95)"] = median_width_0_95
                wandb_log["prob1_TEST(0.95)"] = prob_1_95
                wandb_log["median_width1_TEST(0.95)"] = median_width_1_95
                wandb_log["prob0_TEST(0.99)"] = prob_0_99
...
def compute_interval( confidence_level, po_samples, y_true):
    counter = 0
    width_list = []
    for i in range(po_samples.shape[0]):
        lower_quantile, upper_quantile, in_quantiles = check_interval(confidence_level=confidence_level, y_pred= po_samples[i, :], y_true=y_true[i])
